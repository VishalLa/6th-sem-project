...
async def load_user_transactions(tenant_id: str, db: AsyncSession) -> pd.DataFrame:
    ...
    Transactions are scoped by FileBatch; we join through FileBatch to filter by tenant.
...
async def load_fraud_summaries(tenant_id: str, db: AsyncSession) -> Optional[pd.DataFrame]:
    ...
    FraudRingSummary is scoped by batch_id (FK to FileBatch); we join through
    FileBatch to filter by tenant_user_id.
    ...
    df = pd.DataFrame([{
        ...
        "member_accounts": s.member_accounts,  # Text field with account list
...
async def index_fraud_summaries(
    ...
    Args:
        tenant_id: User ID
        batch_id: FileBatch UUID (required by FAISSVectorStore.add_documents)
        db: Database session
    ...
    # Add to vector store — batch_id is now required by FAISSVectorStore
    ...
    logger.info(f"Indexed {len(documents)} fraud summaries for tenant {tenant_id} (batch {batch_id})")
    return stats
...
async def get_or_create_chatbot(
    ...
    Uses FileBatch-aware queries for both transactions and fraud summaries.
    ...
    # Resolve latest batch_id for vector operations
    batch_id = await get_latest_batch_id(tenant_id, db)

    # Initialize chatbot
    logger.info(f"Creating new chatbot for tenant {tenant_id} (batch {batch_id})")
    ...
    # Index fraud summaries if available and we have a batch_id
    if VECTOR_AVAILABLE and fraud_df is not None and batch_id is not None:
        try:
            fraud_stats = await index_fraud_summaries(
                tenant_id=tenant_id,
                batch_id=batch_id,
                db=db,
